...
class GateDetector(Node):
    ...
    def detect_gate(self, image):
        ...
        if largest_gate:
            ...
            print(f">>>>red x1 x2 {red_x1} {red_x2} red y1 y2 {red_y1} {red_y2}")
            largest_gate = red_area
            center_x_gate = (red_x1 + red_x2) // 2
            center_y_gate = (red_y1 + red_y2) // 2
            cv2.circle(frame, (center_x_gate, center_y_gate), 5, (0, 0, 255), -1)  #
            self.publisher.publish(self.br.cv2_to_imgmsg(frame))
...
def detect_stop(image):
    ...
    for i in range(1):
        ...
        print(area)
        if area > 2000:  # Threshold for a "big" red area
            print(area)
